...
class BoundedEqualityMixin:
    ...
    def equal(self, share_x, share_y, l):
        """Equality testing with secret shared result.
        input [x], [y], and bit length
        """
        ...
        r = r_modl + 2**l*r_divl
        c = self.open(share_x - share_y + r + 2**l)
        return self.schedule_callback(c, self.finish_equal, r_bits)

    def finish_equal(self, c, r_bits):
